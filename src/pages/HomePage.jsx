...
import { useGetUserProfileQuery } from "../store/user/userApiSlice";
...
const HomePage = () => {
  ...
  const {
    ...
    q: '"popular stories" "popular podcasts" "kids-stories" "language":"tamil" "telugu" "hindi" "english"',
    type: "show",
  ...
  const {
    ...
    q: '"top stories" "top podcasts" "kids-stories" "language":"tamil" "telugu" "hindi" "english"',
    type: "show",
    include_external: "audio",
    market: "IN",
    limit: 25,
  ...
  const {
    data: showsOfWeekData,
    isLoading: showsOfWeekLoading,
  } = useGetShowsOfTheWeekQuery({
    q: '"shows of the week" "language":"tamil" "telugu" "hindi" "english"',
    type: "show",
    include_external: "audio",
    market: "IN",
    limit: 25,
  ...
  useEffect(() => {
    ...
    if (showsOfWeekData) {
      ...
      setShowsOfWeek(nonExplicitShowsOfWeek.slice(0, 3));
    }
  }, [popularShowsData, topStoriesData, showsOfWeekData]);

  useEffect(() => {
    if (userData) {
      dispatch(setUserProfile({ profileData: userData }));
    }
  }, [userData, dispatch]);
