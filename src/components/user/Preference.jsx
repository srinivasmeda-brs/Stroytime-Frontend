...
const Preference = () => {
  ...
  const [updateLanguageAPI, { isLoading: languageUpdateLoading }] = useUpdateLanguageAPIMutation();
  ...
  const submitHandler = async () => {
    try {
      const response = await updateLanguageAPI({
        language: userData.languages, // Align key with backend expectation
      }).unwrap();
  ...
  className={`bg-transparent border p-4 rounded-lg hover:bg-active group active text-center ${
    isLanguageSelected(language._id)
      ? " bg-sky-500"
      : "bg-blue border" // Sky blue background for inactive languages
  }`}
  onClick={() => handleLanguageClick(language._id)}
>
  <p className="line-clamp-2 text-link text-xl mt-1">{language.name}</p>
</div>
                </div>
              ))
            ) : (
